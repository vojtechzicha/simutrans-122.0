	...
	lbl_filter("Line Filter")
{
	...
	bt_duplicate_line.init(button_t::roundbox, "Duplicate",
		scr_coord(RIGHT_COLUMN_OFFSET+2*(D_BUTTON_WIDTH+D_H_SPACE), bt_y), scr_size(D_BUTTON_WIDTH, D_BUTTON_HEIGHT));
	bt_duplicate_line.set_tooltip("New line with the same schedule and no convois");
	bt_duplicate_line.set_visible(false);
	bt_duplicate_line.add_listener(this);
	add_component(&bt_duplicate_line);
...
bool schedule_list_gui_t::action_triggered( gui_action_creator_t *comp, value_t v )
{
	...
	else if(  comp == &bt_duplicate_line  ) {
		if(  line.is_bound()  ) {
			tool_t *tmp_tool = create_tool( TOOL_CHANGE_LINE | SIMPLE_TOOL );
			cbuffer_t buf;
			buf.printf( "D,%i", line.get_id() );
...
void schedule_list_gui_t::draw(scr_coord pos, scr_size size)
{
	...
	if(  old_player != welt->get_active_player()  ) {
		...
		bt_duplicate_line.enable( activate );
	}
...
void schedule_list_gui_t::update_lineinfo(linehandle_t new_line)
{
	...
	bt_duplicate_line.set_visible( line.is_bound() );
